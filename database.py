...
def init_db():
    with lock:
        ...
        """)
        # Eski bazalarda "type" ustuni bo'lmasligi mumkin - qo'shib qo'yamiz
        try:
        ...
        """)
        # Eski bazalarda "caption" ustuni bo'lmasligi mumkin - qo'shib qo'yamiz
        try:
...
# ---------- channels / bots (majburiy ro'yxat) ----------
...
def remove_channel(channel_id):
    with lock:
        ...
        cur.execute("DELETE FROM bot_confirmations WHERE channel_id = ?", (channel_id,))
        conn.commit()
        conn.close()
...
# ---------- bot tasdiqlari (self-confirm) ----------
def confirm_bot(user_id, channel_id):
    with lock:
        conn = get_conn()
        cur = conn.cursor()
        cur.execute(
            "INSERT OR IGNORE INTO bot_confirmations (user_id, channel_id) VALUES (?, ?)",
            (user_id, channel_id),
        )
        conn.commit()
        conn.close()
...
def is_bot_confirmed(user_id, channel_id):
    with lock:
        conn = get_conn()
        cur = conn.cursor()
        cur.execute(
            "SELECT 1 FROM bot_confirmations WHERE user_id = ? AND channel_id = ?",
            (user_id, channel_id),
        )
        row = cur.fetchone()
        conn.close()
