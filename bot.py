...
def adm_addchannel(call):
    ...
    if type_ == "channel":
        bot.send_message(
            call.message.chat.id,
            "📢 Kanal username yoki invite link yuboring:\n\n"
            "📌 Ochiq kanal: @kanal_username\n"
            "📌 Yopiq kanal: https://t.me/+abcd1234 (invite link)\n\n"
...
def process_add_channel(message):
    ...
    # Invite link yoki username yoki ID ni aniqlash
    invite_link = None
    ...
    if user_input.startswith('https://t.me/') or user_input.startswith('t.me/'):
        ...
        # Linkdan kanal ID'sini olishga urinamiz
        try:
            # Invite link orqali kanal ma'lumotini olish
            # Bu yerda invite linkni saqlaymiz, lekin ID'sini topish uchun
            # bot.get_chat() ishlamaydi, shuning uchun username sifatida saqlaymiz
            # yoki ID ni aniqlash uchun boshqa usul
            pass
        except:
            pass
        ...
        # Username deb hisoblaymiz
        channel_id = '@' + user_input if not user_input.startswith('@') else user_input
    
    # Kanal nomini olishga urinamiz
    title = channel_id
    try:
        # Agar username bo'lsa, chat ma'lumotini olish
        if not invite_link:
            chat = bot.get_chat(channel_id)
            title = chat.title or channel_id
    except Exception as e:
        # Yopiq kanal bo'lsa, invite_link dan foydalanamiz
        if invite_link:
            title = "Yopiq kanal"
        else:
            bot.send_message(
                ...
                f"Xatolik: {str(e)}\n\n"
                f"Kanali ID: {channel_id}\n"
                f"Kanal qo'shildi, lekin tekshirish ishlamasligi mumkin."
            )
