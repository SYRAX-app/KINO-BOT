...
# Adminning hozirgi holatini saqlab turish uchun (kanal qo'shyaptimi, video kod kiritayaptimi va h.k.)
...
def check_subscription(user_id):
    ...
    Bajarilmagan punktlar ro'yxatini qaytaradi: (channel_id, title, type)"""
    items = db.get_channels()
    not_done = []
    for channel_id, title, type_ in items:
        ...
        if type_ == "channel":
            try:
                ...
                # Bot o'sha kanalda admin bo'lmasa yoki kanal noto'g'ri kiritilgan bo'lsa - o'tkazib yuboramiz
                continue
        else:  # type_ == "bot"
            # Telegram API boshqa botga /start bosilganini tekshirishga imkon bermaydi,
            # shu sabab foydalanuvchi o'zi "Tasdiqlayman" tugmasini bosishi kerak.
            if not db.is_bot_confirmed(user_id, channel_id):
                not_done.append((channel_id, title, type_))
    return not_done
...
def subscription_keyboard(not_done):
    ...
    for channel_id, title, type_ in not_done:
        username = str(channel_id).lstrip('@')
        url = f"https://t.me/{username}"
        if type_ == "channel":
            markup.add(types.InlineKeyboardButton(f"📢 {title or channel_id}", url=url))
        else:
            markup.add(types.InlineKeyboardButton(f"🤖 {title or channel_id}", url=url))
            markup.add(types.InlineKeyboardButton(
                f"☑️ {title or channel_id} — tasdiqlayman",
                callback_data=f"botconfirm_{channel_id}",
            ))
...
def start_handler(message):
    ...
    bot.send_message(
        message.chat.id,
        "👋 Xush kelibsiz!\n\nKino olish uchun menga kodni yuboring.\nMasalan: 130-677",
    )
...
def check_sub_callback(call):
    ...
    if not_done:
        bot.answer_callback_query(call.id, "❌ Siz hali barcha shartlarni bajarmadingiz!", show_alert=True)
        return
...
@bot.callback_query_handler(func=lambda call: call.data.startswith("botconfirm_"))
def botconfirm_callback(call):
    channel_id = call.data.replace("botconfirm_", "", 1)
    db.confirm_bot(call.from_user.id, channel_id)
    bot.answer_callback_query(call.id, "☑️ Tasdiqlandi! Endi \"✅ Tekshirish\" tugmasini bosing.")
...
def adm_channels(call):
    ...
    if channels:
        for cid, title, type_ in channels:
            icon = "📢" if (type_ or "channel") == "channel" else "🤖"
            text += f"{icon} {title or ''} ({cid})\n"
    else:
...
def adm_addchannel(call):
    ...
    if type_ == "channel":
        ...
        bot.send_message(
            ...
            "⚠️ Diqqat: Telegram boshqa botga /start bosilganini tekshirishga imkon bermaydi. "
            "Foydalanuvchi botni ochib, o'zi \"Tasdiqlayman\" tugmasini bosadi.",
        )
...
def adm_delchannel(call):
    ...
    for cid, title, type_ in channels:
        icon = "📢" if (type_ or "channel") == "channel" else "🤖"
        markup.add(types.InlineKeyboardButton(f"❌ {icon} {title or cid}", callback_data=f"delch_{cid}"))
    markup.add(types.InlineKeyboardButton("⬅️ Orqaga", callback_data="adm_channels"))
    bot.answer_callback_query(call.id)
    bot.edit_message_text("O'chirmoqchi bo'lgan punktni tanlang:", call.message.chat.id, call.message.message_id, reply_markup=markup)
